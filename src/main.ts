...
export interface Rover {
  ...
  energy: number;
  maxEnergy: number;
  assignedTargetIndex: number | null;
  path: Cell[];
  completed: boolean;
  failed: boolean;
  color: string;
...
export interface MissionMetrics {
  success: boolean;
  roverResults: RoverResult[];
  missionTargetsReached: number;
  totalTargets: number;
  totalExploredNodes: number;
  totalPathLength: number;
  totalEnergyUsed: number;
  averageTerrainCost: number;
}
...
function getRandomEmptyCell(grid: Cell[][], avoidRadius = 2): Cell | null {
...
function placeTargets(grid: Cell[][], count: number): MissionTarget[] {
...
function createRovers(count: number, roverEnergy: number): Rover[] {
  const rovers: Rover[] = [];
...
function findPath(
  ...
  goalX: number,
  goalY: number
): { path: Cell[]; exploredNodes: number; totalCost: number; solved: boolean } {
  const open: Array<{ x: number; y: number; f: number }> = [
    { x: startX, y: startY, f: 0 },
  ];
...
function assignTargets(rovers: Rover[], targets: MissionTarget[]): void {
...
function stampMissionPaths(grid: Cell[][], rovers: Rover[]): void {
  for (const rover of rovers) {
...
function runMission(
  ...
  const roverResults: RoverResult[] = [];
...
function terrainColor(cell: Cell): string {
...
function terrainLabel(cell: Cell): string {
  switch (cell.type) {
    case "hazard":
      return "H";
    case "start":
...
function createMetricCard(title: string, value: string): HTMLDivElement {
